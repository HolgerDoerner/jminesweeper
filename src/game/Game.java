...
public class Game {
  ...
  /**
   * <p>
   * calculates the fields of an level and updates the array before the game starts.
   * </p>
   * <p>
   * it traverses an two-dimensional array linear and checks the neighbor indexes for bombs. if it
   * finds one it updates the current index and increases the value by 1.
   * </p>
   */
  ...
  /**
   * reveals a field and checks if neighbor fields can also be revealed. if this is the case it
   * calls itself in a new process for the neighbor field and so on until no neighbor field is left
   * to reveal.
   *
  ...
  public static synchronized void revealField(final int positionY, final int positionX) {
    ...
    if (level.get(positionY, positionX) >= 'A' & level.get(positionY, positionX) <= 'I') {
      return;
    } else if (level.get(positionY, positionX) >= 'a' & level.get(positionY, positionX) <= 'i') {
      return;
        ...
        try {
          if (level.get(positionY, positionX + i) == BOMB) {
            ...
            ;
              ...
              & level.get(positionY, positionX) <= 'i') {
            ;
          } else if (level.get(positionY, positionX + i) >= 'A'
              & level.get(positionY, positionX) <= 'I') {
            ;
  ...
  /**
   * saves the current level to a file on disc and produces some kind of 'savegame'. if the file
   * already exists it will be ovewritten, if not a new file will be created.
   */
  ...
  /**
   * <p>
   * start a new game and present a dialog letting the player make the desired settings.
   * </p>
   * <p>
   ...
   *
   * @param args arguments passed to the main programm
   * @throws InterruptedException on interrupt
   * @throws BrokenBarrierException on exception in CyclicBarrier
   */
