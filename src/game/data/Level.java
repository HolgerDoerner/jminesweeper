...
public interface Level {
  ...
  public static Level fromExistingData(char[][] levelData) {
    ...
    return new Level() {
      private final char[][] level = levelData.clone();

      @Override
      public void set(int positionY, int positionX, char value) {
        level[positionY][positionX] = value;
      }
      ...
      public char get(int positionY, int positionX) {
        return level[positionY][positionX];
      }
  ...
  public static Level generateNew(int y, int x, int numBombs) {
    ...
    while (bombcount < numBombs) {
      for (int i = 0; i < y; i++) {
        for (int j = 0; j < x; j++) {
          if ((ThreadLocalRandom.current().nextInt(0, numBombs) % fieldsize == 0)
              && (bombcount < numBombs) && levelData[i][j] != Game.BOMB) {
            levelData[i][j] = Game.BOMB;
            bombcount++;
          }
        }
      }
