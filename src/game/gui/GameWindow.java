...
public class GameWindow extends JFrame implements Runnable {
    ...
    private Field(int y, int x) {
      ...
      this.addMouseListener(new MouseAdapter() {
        ...
        public void mouseReleased(MouseEvent e) {
          if (e.getButton() == MouseEvent.BUTTON1 & self.active) { // left mouse-button
            Game.revealField(self.positionY, self.positionX);
            // self.active = false;
            // self.clicked = true;
          } else if (e.getButton() == MouseEvent.BUTTON3 & self.active) { // right mouse-button
            Game.markField(self.positionY, self.positionX);
          }
          ...
          if (Game.DEBUG) {
            System.out.println("Clicked Field: " + self.getPositionY() + "x"
                + self.getPositionX() + "\tMouse-Button: " + e.getButton());
          }
        }
  ...
  /**
   * returns a java.util.Map containing the fields of the current board.
   *
  ...
  public void updateTouchedFields(final Map<String, Character> touchedFields) {
    touchedFields.entrySet().forEach(entry -> Game.threadPool
        .execute(() -> gameFields.get(entry.getKey()).updateField(entry.getValue())));
  }
   ...
   * 
   * @param time the elapsed gametime in seconds
   */
  ...
  /**
   * updates the bomb-counter in the GUI.
   * 
   * @param numBombs the number of bombs left
   */
  ...
  /**
   * generates a new board inside of the main-window. should a board already exist, it gets deleted.
   *
